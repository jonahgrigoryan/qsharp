...
class ParallelExt(Parallel):
    def __init__(self, *args, **kwargs):
        ...
        if isinstance(self._backend, MultiprocessingBackend) and maxtasksperchild is not None:
            # Handle joblib API changes gracefully
            if hasattr(self, "_backend_args"):
                self._backend_args["maxtasksperchild"] = maxtasksperchild  # type: ignore[attr-defined]
            elif hasattr(self, "_backend_kwargs"):
                self._backend_kwargs["maxtasksperchild"] = maxtasksperchild  # type: ignore[attr-defined]
            # else: attribute no longer exists (joblib>=1.4); silently ignore the setting to stay compatible.
